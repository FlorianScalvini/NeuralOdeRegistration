...
class SpatioTemporalDatasetJSON(torch.utils.data.dataset.Dataset):
    def __init__(self, root_path, json_path, transform=None):
        ...
        for i in range(len(data)):
            ...
            for j in range(len(data[i]['session'])):
                session = [
                    self.root_path + data[i]['session'][j]['image'],
                    self.root_path + data[i]['session'][j]['segmentation'],
                    data[i]['session'][j]['age'],
                ]
            ...
            for i in range(len(subject)):
                subject[i][2] = ((subject[i][2] / 7.0 - t0) / (tn - t0))
            if len(subject) == 1:
                self.single_session.append(subject)
            elif len(subject) == 2:
                self.multi_session.append(subject)
            #IGNORE >3 (val)
