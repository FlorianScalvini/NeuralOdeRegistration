...
from torch.ao.nn.quantized import InstanceNorm3d
...
class FinalPatchExpand3D(nn.Module):
    def __init__(self, dim, out_channels=3, patch_size=4):
        super().__init__()
...
class Encoder(nn.Module):
    def __init__(self, in_channels=1, embed_dim=64,
        ...
        # t₀  →  cond vector (same dim as first embed stage)
        ...
        self.pos_embd = SinPositionalEncoding3D(channels=embed_dim)
        self.pos_drop = nn.Dropout(p=0.)

        dpr = [x.item() for x in torch.linspace(0, drop_path_rate, sum(depths))]

        self.layers = nn.ModuleList()
        for i in range(self.num_layers):       # cond vector stays at embed_dim throughout
            self.layers.append(BasicLayer(
                dim=int(embed_dim * 2 ** i), depth=depths[i],
    ...
    def forward(self, image: torch.Tensor) -> List[List[torch.Tensor]]:
        """
        image : (B, 1, H, W, D)
        t    : (B, L)   acquisition age (scalar per subject)
        returns: list of multi-scale feature maps
        """
        # cond vector from acquisition age — stays fixed for all layers


        x    = self.patch_embed(image)                     # (B, C, Wh, Ww, Wt)
        x    = (x + self.pos_embd(x)).flatten(2).transpose(1, 2)   # (B, L, C)
        x    = self.pos_drop(x)
        Wh, Ww, Wt = (image.shape[2] // self.patch_embed.patch_size[0],
                       image.shape[3] // self.patch_embed.patch_size[1],
                       image.shape[4] // self.patch_embed.patch_size[2])
        ...
        for i, layer in enumerate(self.layers):
            ...
            if i in self.out_indices:
                ...
                x_normed = norm(x_out)
                out = x_normed.view(-1, H, W, T, self.num_features[i]).permute(0,4,1,2,3).contiguous()
                outs.append(out)
        return outs   # list of (B, C_i, H_i, W_i, T_i)
