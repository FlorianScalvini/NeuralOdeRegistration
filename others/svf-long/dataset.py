...
class SpatioTemporalDataset(torch.utils.data.dataset.Dataset):
    ...
    def __getitem__(self, idx: int) -> tio.Subject:
        ...
        ## Crop the sequence to variable length in order to vary the starting point of the sequence during training (data augmentation)
        if len(all_times) > 2:
            start_idx = torch.randint(0, len(all_times) - 1, (1,)).item()
            mri_stack_out = mri_stack_out[start_idx:]
            seg_stack_out = seg_stack_out[start_idx:] if len(all_seg) > 0 else seg_stack_out
            time_stack_out = time_stack_out[start_idx:]
            is_mono_out = is_mono_out[start_idx:]  
            sdf_stack_out = sdf_stack_out[start_idx:]
